...
def pre_reading_eeg():
    for files_key in files_by_label.keys():
        for (idx, file) in enumerate(files_by_label[files_key]):
            ...
            # reshape to (segments, chs, 30, 200)
            eeg = eeg.reshape(chs, -1, 30, 200).transpose(1, 0, 2, 3)
...
def fold_construct(fold_idx):
    ...
    db = lmdb.open(f'/mnt/disk1/aiotlab/namth/EEGFoundationModel/datasets/pearl_30s/fold_{fold_idx}', map_size=1000000000)
